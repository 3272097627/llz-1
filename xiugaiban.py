...
class CementProductionModel:
    ...
    def define_constants(self):
        ...
        # 反应焓变
        self.constants['deltaHr1_T'] = 170e3  # r1反应焓变，J/mol
    ...
    def configure_sections(self):
        ...
        self.section_configs['kiln'] = {
            ...
            'reactions': ['r2', 'r3', 'r4', 'r5', 'r6', 'r7', 'r8', 'r9', 'r10', 'r11'],  # r2~r11熟料反应
    ...
    def spatial_discretization(self):
        ...
        for i in range(total_segments):
            ...
            # 初始化代数变量
            # T: 系统温度向量，初始化为300K
            self.variables['algebraic_variables']['T'].append(300.0)
        ...
        for i in range(total_segments): #再次遍历所有离散单元，构建单元字典并添加到cells列表
            ...
            cell = {
                ...
                'algebraic_variables': { #单元的代数变量子字典
                    'T': self.variables['algebraic_variables']['T'][i],
    ...
    def formula_11(self, M_i, C_i_g):
        ...
        if isinstance(M_i, dict) and isinstance(C_i_g, dict):
            ...
            return sum(M_i * C_i for M_i, C_i in zip(M_i, C_i_g))
    ...
    def formula_18(self, A_s, A_t, eta):
        # 18填充率和填充角迭代计算
        # 输入：As（公式16输出）、At（公式17输出）、η（填充因子，变量）
        # 输出：θ(z)（填充角，变量）
        # 公式：η = A_s / A_t = (θ - sinθ)/(2π)
        # 使用牛顿迭代法求解填充角
        
        # 1. 确定使用的填充率
        # 如果eta是初始值或未提供，使用面积比；否则使用传入的eta值
        # 初始填充率设置为0.13
    ...
    def formula_21(self, vg, C_i_g, T, P, components, xj, cg, dC_i_g_dz):
        # 21气体组分i物质通量
        # 输入：vg（公式10输出）、Ci,g（组分i气体组分浓度，变量）、T（温度）、P（压力）、components（组分列表）、xj（各组分摩尔分数）、cg（气体总摩尔浓度）、∂zCi,g（组分i浓度梯度，公式23输入）
        # 输出：Ni,g（气体组分i物质通量）
        # 公式：N_i,g = v_g * C_i,g - D_i,g * ∂_z C_i,g
        # 计算有效扩散系数
        D_i_g = self.formula_22(T, P, components, xj, cg)
        return vg * C_i_g - D_i_g * dC_i_g_dz
    
    def formula_22(self, T, P, components, xj, cg):
        # 22气体有效扩散系数
        # 输入：T（温度，变量）、P（压力，变量）、components（组分列表）、xj（各组分摩尔分数列表，公式61输出）、cg（气体总摩尔浓度，变量，公式77输出）
        # 输出：Di,g（气体有效扩散系数）
        # 公式：D_i,g = (Σ_{j≠i} x_j / (c_g D_ij))^(-1)
        sum_term = 0.0
        i = 0  # 假设当前计算的是第一个组分的有效扩散系数
    ...
    # 反应动力学与质量守恒方程
    def formula_26(self, reaction_id, T, P_i_dict, C_i_dict):
        # 26反应速率
        # 输入：reaction_id（反应标识，如'r1'）、T（温度，变量）、P_i_dict（组分分压字典）、C_i_dict（组分浓度字典）
        # 输出：rj（反应速率）
        ...
        # 根据反应标识匹配反应物组分
        # 固体组分列表（前9个）：CaCO3, CaO, SiO2, Al2O3, Fe2O3, C2S, C3S, C3A, C4AF
        # 气体组分列表（后6个）：C_sus, CO2, CO, O2, H2O, H2
        components = self.constants['stoichiometric_matrix']['components']
        
        # 根据反应ID确定反应物顺序
        # 这里假设alpha1、alpha2、alpha3对应不同的反应物，需要根据具体反应调整
        # 由于没有具体的反应物对应关系，这里简化处理，使用浓度最高的前几个组分
        # 实际应用中需要根据具体反应确定反应物顺序
        ...
        # 假设beta2对应O2的分压指数
        if 'O2' in P_i_dict:
            product_P *= P_i_dict['O2'] ** beta2
        
        # 计算浓度乘积项 ∏ C_i^α_i
        product_C = 1.0
        # 假设alpha1、alpha2、alpha3对应不同的反应物浓度指数
        # 这里需要根据具体反应调整，暂时使用前三个组分作为示例
        for i, alpha in enumerate([alpha1, alpha2, alpha3]):
            if i < len(components) and components[i] in C_i_dict:
                product_C *= C_i_dict[components[i]] ** alpha
        
        # 计算反应速率
        rj = kr * (T ** n) * math.exp(-EA / (R * T)) * product_P * product_C
        return rj
    
    def formula_27(self, T, P_i_dict, C_i_dict):
        # 27反应源项计算（固/气相生成速率）
        # 输入：T（温度，变量）、P_i_dict（组分分压字典）、C_i_dict（组分浓度字典）
        # 输出：Rs,Rg（固/气相生成速率）
        # 公式：[R_s; R_g] = v * r
        ...
        for reaction_id in reactions:
            rj = self.formula_26(reaction_id, T, P_i_dict, C_i_dict)
            r.append(rj)
    ...
    def formula_42(self, H_tilde_s, H_tilde_g, Q_tilde_g, Qgscv, Qgsrad, V_delta):
        # 42分解炉能量平衡
        # 输入：H~s,H~g（气体\固体焓通量密度，公式68、69输出）、Q~g（气体热传导，公式54输出）、Qgscv,Qgsrad（对流辐射换热，公式35、41输出）、VΔ（单段总体积，公式58输出）
        # 输出：∂tU^c（分解炉内能密度变化率）
        # 公式：∂_t Û_c = -∂_z (H~_g + H~_s + Q~_g) - (Qgs^rad + Qgs^cv) / VΔ
        # 这里假设H_tilde_s、H_tilde_g和Q_tilde_g已经包含了梯度计算
        return -(H_tilde_g + H_tilde_s + Q_tilde_g) - (Qgsrad + Qgscv) / V_delta
    ...
    def formula_44(self, H_tilde_s, Q_tilde_s, Qgsrad, Qgscv, V_delta, Jsg):
        # 44回转窑固体能量平衡
        # 输入：H~s（固体焓通量密度，公式68输出）、Q~s（热传导，公式55输出）、Qgsrad,Qgscv（公式39、41输出）、VΔ（单段总体积，公式58输出）、Jsg（反应焓变，公式52输出）
        # 输出：∂tU^s（固体内能密度变化率）
        # 公式：∂_t Û_s = -∂_z (H~_s + Q~_s) + (Qgs^rad + Qgs^cv) / VΔ - J_sg
        # 这里假设H_tilde_s和Q_tilde_s已经包含了梯度计算
        return -(H_tilde_s + Q_tilde_s) + (Qgsrad + Qgscv) / V_delta - Jsg
    
    def formula_45(self, H_tilde_g, Q_tilde_g, Qgsrad, Qgscv, V_delta, Jsg):
        # 45回转窑气体能量平衡
        # 输入：H~g（气体焓通量密度，公式69输出）、Q~g（热传导，公式54输出）、Qgsrad,Qgscv（公式39、41输出）、VΔ（单段总体积，公式58输出）、Jsg（反应焓变，公式52输出）
        # 输出：∂tU^g（气体内能密度变化率）
        # 公式：∂_t Û_g = -∂_z (H~_g + Q~_g) - (Qgs^rad + Qgs^cv) / VΔ + J_sg
        # 这里假设H_tilde_g和Q_tilde_g已经包含了梯度计算
        return -(H_tilde_g + Q_tilde_g) - (Qgsrad + Qgscv) / V_delta + Jsg
    ...
    def formula_52(self, r1, deltaHr1_T):
        # 52反应焓变计算
        # 输入：r1（反应速率）、ΔHr1(T)（r1的反应焓，参数）
        # 输出：Jsg（反应焓变）
        # 公式：J_sg = r1 × ΔH_r1(T)
        return r1 * deltaHr1_T
    ...
    def formula_55(self, k_s, dT_i_dz):
        # 55固体热传导
        # 输入：ks（固体热导率，常数）、∂Ti/∂z（温度梯度，公式43输出）
        # 输出：Q~s（固体热传导）
        # 公式：Q~_s = -k_s ∂T_s/∂z
        return -k_s * dT_i_dz
    ...
    def solve(self):
        ...
        while not self.converged and iteration < self.max_iterations:
            ...
            if self.check_convergence(prev_core_vars, current_core_vars): #对比迭代前后的核心变量，判断差值是否均小于收敛阈值（1e-6）
                self.converged = True
...
#一、基础几何与填充参数计算  
        ...
        # 使用初始填充率计算填充角（公式18）
        # 使用A_s=0表示使用填充率计算填充角
        theta = self.formula_18(0, A_t, initial_eta)
        
        # 使用计算得到的填充角计算固体横截面积（公式16）
        A_s = self.formula_16(r_c, theta)
        
        # 计算实际填充率（公式：η = A_s / A_t）
        eta = A_s / A_t
...
#二、初始温度与压力设置   
        ...
        #判断单元的代数变量中是否已存在温度 T;若存在，直接读取上一次的温度值
        if 'T' in cell['algebraic_variables']:
            T = cell['algebraic_variables']['T']
        else:
            # 第一次迭代根据设备类型设置合理的初始温度
            if device_type == 'preheater':
                T = 390.0  # 预热器初始温度，对应固体进料温度
        ...
        # 限制温度范围，避免数值溢出
        T = max(T, 300.0)  # 最低温度300K
        T = min(T, 2000.0)  # 最高温度2000K
...
#三、组分摩尔焓计算      
        ...
        for component in solid_components: #遍历所有固体组分，逐个计算摩尔焓
            ...
            # 计算积分焓（公式5）
            integral_enthalpy = self.formula_5(C0, C1, C2, T0, T)
        ...
        for component in gas_components:
            ...
            # 计算摩尔焓（公式50）
            h_i = self.formula_50(Hf_i, integral_enthalpy)
            h_i_g.append(h_i) 3将气体组分的摩尔焓添加到h_i_g列表

#四、物质通量计算        
        # 2. 计算各组分的通量
        # 获取速度，使用差异化配置中的固相速度公式
        vg = algebraic_vars.get('vg', 0.084)  # 气体速度
        ...
        if index > 0: #若不是第一个单元
            ...
            dC_i_g_dz = self.formula_23(C[component], prev_C_i_g, delta_z)
        
        for component in gas_components: #遍历所有气体组分
            C_i_g = C[component]
            N_i = self.formula_21(vg, C_i_g, T, P, components, xj, cg, dC_i_g_dz)
            N_i_g.append(N_i)
...
#六、气体比热容与对流换热计算      
        ...
        for component in gas_components: #遍历所有气体组分
            ...
            # 计算摩尔热容（公式6）
            cp_i = self.formula_6(C0, C1, C2, T)
        ...
        # 使用公式81计算出的cp_g
        mu_g = self.constants.get('gas_viscosity', 4.5e-5)  # 气体平均粘度，Pa·s
        # 计算气体热导率（公式20）
        k_g = self.formula_20(T)
        # 计算普朗特数
        ...
        if isinstance(heat_transfer_rule, str) and heat_transfer_rule == 'formula_35':
            ...
            # 计算气体密度（公式11）
            M_i = self.constants['gas_properties']  # 气体组分摩尔质量
            C_i_g = C  # 气体组分浓度
            rho_g = self.formula_11(M_i, C_i_g)
            ...
            # 计算努塞尔数（公式37）
            Nu = self.formula_37(ReD, Re_omega, initial_eta)
            ...
            # 计算对流换热量（公式39）
            Qgscv = self.formula_39(Ags, beta, T, Ts)
...
#七、内能密度约束计算
        ...
        for component in solid_components: #遍历固体组分，准备总焓计算参数
            ...
            integral_h = self.formula_5(C0, C1, C2, T0, T)
        ...
        V_g = self.formula_7(self.constants['R'], T, P, gas_moles)
    ...
    def _solve_differential_equations(self, cell, rules, algebraic_vars):
        ...
        # 获取从代数计算得到的约束变量
        T = algebraic_vars.get('T', 1000.0)  # 温度，从代数计算得到
...
#一、反应源项计算        
        ...
        if not reactions: #判断反应列表是否为空
            if section == 'calciner':
                reactions = ['r1']  # 分解炉仅r1分解反应
            elif section == 'kiln':
                reactions = ['r2', 'r3', 'r4', 'r5', 'r6', 'r7', 'r8', 'r9', 'r10', 'r11']  # 回转窑r2~r11熟料反应
            else:
                reactions = []  # 其他设备无反应
        ...
        try: #异常捕获起始，确保无论计算是否出错，都能恢复原始反应列表
            ...
            Rs, Rg = self.formula_27(T, P_i_dict, C_i_dict)
...
#二、浓度变化率计算     
        ...
        for i, component in enumerate(gas_components): #遍历所有气体组分，逐个计算浓度变化率
            # 计算气体通量（公式21）
            N_i_g = self.formula_21(vg, C[component], T, P, components, xj, cg, dC_i_g_dz)
            ...
            if index > 0:
                ...
                prev_N_i_g = self.formula_21(prev_vg, prev_C[component], T, P, components, xj, cg, dC_i_g_dz)
...
#三、内能变化率计算            
        ...
        if isinstance(energy_eq, str):
            ...
            if energy_eq == 'formula_42':
                ...
                epsilon_g = self.formula_53(T, P, xH2O, xCO2, r_c)
                ...
                # 计算辐射换热量
                Qgsrad = self.formula_41(sigma, Ags, epsilon_gs, T, T)  # 假设Ts=T
                
                # 计算温度梯度（公式43）
                # 获取当前单元和前一个单元的温度
                current_T = T
                prev_T = current_T #默认前序单元温度与当前一致
                if index > 0: #若不是第一个单元
                    prev_cell = self.cells[index-1] #获取前一个单元
                    prev_T = prev_cell['algebraic_variables'].get('T', current_T) #读取前一个单元的温度
                # 构建温度列表用于计算梯度
                T_list = [prev_T, current_T]
                dT_i_dz = self.formula_43(T_list)
                
                # 计算气体热传导（公式54），使用计算得到的温度梯度
                k_g = self.formula_20(T)
                Q_tilde_g = self.formula_54(k_g, dT_i_dz)
                ...
                # 计算分解炉内能变化率
                dU_dt = self.formula_42(H_tilde_s, H_tilde_g, Q_tilde_g, Qgscv, Qgsrad, V_delta)
            ...
            if len(energy_eq) >= 2 and energy_eq[0] == 'formula_44' and energy_eq[1] == 'formula_45':
                ...
                Lc = self.formula_14(r_c, theta)
                Ags = self.formula_40(Lc, cell['delta_z'])
                Qgsrad = self.formula_41(sigma, Ags, epsilon_gs, T, T)  # 辐射换热量
                ...
                # 获取当前单元和前一个单元的温度
                current_T = T
                prev_T = current_T #前序温度=当前温度
                if index > 0:
                    prev_cell = self.cells[index-1] #前一个单元
                    prev_T = prev_cell['algebraic_variables'].get('T', current_T) #前序温度
                # 构建温度列表用于计算梯度
                T_list = [prev_T, current_T]
                dT_i_dz = self.formula_43(T_list)
                ...
                # 计算热传导
                k_g = self.formula_20(T)
                k_s = self.constants.get('k_s', 3.5)
                Q_tilde_g = self.formula_54(k_g, dT_i_dz)
                Q_tilde_s = self.formula_55(k_s, dT_i_dz)
                
                # 计算反应焓变（公式52）
                Jsg = 0.0
                if reactions and 'r1' in reactions:
                    # 使用公式26计算r1反应速率
                    P_i_dict = {}  # 组分分压字典
                    C_i_dict = C  # 组分浓度字典
                    r1 = self.formula_26('r1', T, P_i_dict, C_i_dict)
                    deltaHr1_T = self.constants.get('deltaHr1_T', 170e3)
                    Jsg = self.formula_52(r1, deltaHr1_T)
                
                # 计算固体能量变化率
                dU_dt_solid = self.formula_44(H_tilde_s, Q_tilde_s, Qgsrad, Qgscv, V_delta, Jsg)
                
                # 计算气体能量变化率
                dU_dt_gas = self.formula_45(H_tilde_g, Q_tilde_g, Qgsrad, Qgscv, V_delta, Jsg)
                
                # 综合固体和气体的内能变化率
                dU_dt = (dU_dt_solid + dU_dt_gas) / 2
        else:
            # 默认使用分解炉能量方程
            dU_dt = self.formula_42(H_tilde_s, H_tilde_g, 0.0, 0.0, 0.0, cell['delta_z'])
...
#一、变量更新方法   
    def _update_variables(self, cell, dC_dt, dU_dt):
        ...
        # 重新计算温度 T（基于能量守恒）
        # 温度与内能成正比
        if state_vars['U_hat'] > 0:  #判断内能是否为正
            # 更合理的温度更新逻辑，避免温度过快增长
            new_T = algebraic_vars['T'] + (dU_dt * self.dt) * 0.001  # 计算新温度（原温度 + 内能变化 × 修正系数）
            # 限制温度范围
            algebraic_vars['T'] = max(new_T, 300.0)  # 确保温度不低于300K
            algebraic_vars['T'] = min(algebraic_vars['T'], 2000.0)  # 确保温度不高于2000K
        ...
        # 3. 同步更新全局数组中的代数变量
        self.variables['algebraic_variables']['T'][index] = algebraic_vars['T']
...
#二、相邻单元传递方法
    def _transfer_between_cells(self, cell1, cell2):
        ...
        # 复制单元1的代数变量到单元2
        if 'T' in cell1['algebraic_variables']: #判断单元 1 是否有温度变量
            cell2['algebraic_variables']['T'] = cell1['algebraic_variables']['T'] #复制温度值
...
#三、预热器进料补给方法
    def _feed_preheater_inlet(self):
        ...
        if self.cells and self.cells[0]['device_type'] == 'preheater':
            ...
            # 设置初始进料温度
            inlet_cell['algebraic_variables']['T'] = self.control_variables['solid_feed']['temperature']
            ...
            for component, mass_fraction in self.control_variables['solid_feed']['composition'].items(): #遍历固体进料的各组分
                ...
                # 计算浓度（假设初始体积为1）
                inlet_cell['state_variables']['C'][component] = molar_rate
            ...
            for component, mole_fraction in self.control_variables['gas_feed']['composition'].items(): #遍历气体进料的各组分
                # 使用理想气体状态方程计算初始浓度（C = xP/(RT)）
                inlet_cell['state_variables']['C'][component] = mole_fraction * gas_pressure / (R * gas_temp)
...
#五、核心变量获取方法
    def _get_core_variables(self):
        ...
        # 温度、主物料浓度、内能
        core_vars = { #构建核心变量字典
            'gas_temperature': self.variables['algebraic_variables']['T'],
            'solid_temperature': self.variables['algebraic_variables']['T'],  # 简化处理，实际需要区分气固温度
...
def main():
    ...
    model.initialize_variables() #状态变量 空
